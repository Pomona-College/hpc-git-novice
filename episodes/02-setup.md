...
Cecil sets up his new laptop:

```bash
$ git config --global user.name "Cecil Sagehen"
$ git config --global user.email "cecil@pomona.edu"
```

Please use your own name and email address instead of Cecil's. This user name and email will be associated with your subsequent Git activity,
...
Cecil also has to set his favorite text editor, following this table:
...
initialize any new repository.  Cecil decides to use that feature to set it to `main` so
