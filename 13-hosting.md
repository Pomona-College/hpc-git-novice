...
If the answer to all four is "yes, public is fine," then you can typically open-source the repository unilaterally. If any answer is unclear, default to a private repository until you have explicit approval.
...
A practical comparison from a Pomona researcher's perspective:

| Service | Best for | Trade-offs |
|---------|----------|------------|
| **GitHub.com (public)** | Code with broad reach; CI/CD; open source community | No control over the platform; sensitive data not allowed; free tier has LFS limits |
| **GitHub.com (private/Education)** | Code that needs to stay internal but uses GitHub's tooling | Still hosted by Microsoft; not appropriate for FERPA/HIPAA/ITAR data |
| **GitLab.com / self-hosted GitLab** | Teams that want to migrate later; built-in CI; broader self-hosting story | Smaller third-party ecosystem than GitHub; self-hosting is real ops work |
| **Pomona-hosted Sagehen storage** (`/bigdata`) | Anything involving regulated data; very large files | Discoverability is poor; not a true "version control" host; pair with Git on a private remote for full provenance |
| **arXiv / preprint servers** | Manuscripts and figures, before or alongside journal submission | Not a code/data host; immutable record once posted |
| **Figshare / Zenodo / Dryad** | Persistent, citable datasets and code releases (DOIs) | Less interactive than GitHub; intended for archival rather than ongoing development |

A common Pomona pattern: develop code in a **GitHub private repo** while the science is in flight, push raw data to **Sagehen `/bigdata`** so it never leaves College storage, and at publication time create a **GitHub release** that is mirrored to **Zenodo** for a citable DOI. That gives you good day-to-day collaboration tools, a clear audit trail for sensitive data, and a permanent reproducible record after the paper goes out.
